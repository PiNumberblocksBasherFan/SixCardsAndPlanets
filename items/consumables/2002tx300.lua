SMODS.Consumable {
    ...
    in_pool = function(self, args)
        if (G.GAME and G.GAME.starting_params.play_limit >= 5) then
            return true
        end
    ...
    set_card_type_badge = function(self, card, badges)
        badges[#badges + 1] = create_badge(localize('k_mxms_satellite'), get_type_colour(card.config.center, card),
            nil, 1.2)
    end
}
